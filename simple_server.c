#include<stdio.h>
#include<stdlib.h>
#include<string.h>
#include<sys/socket.h>
#include<netinet/in.h>
#include<arpa/inet.h>
#include<unistd.h>
#include"hacking.h"

#define PORT 7890 //Port for user's connection

int main(void)
{
	int sockfd, new_sockfd; //Listening socket in variable sockfd, new connection in new_sockfd
	struct sockaddr_in host_addr, client_addr; //My address data
	socklen_t sin_size;
	int recv_length = 1, yes = 1;
	char buffer[1024];

	if((sockfd = socket(PF_INET, SOCK_STREAM, 0)) == -1)
		fatal("in socket");

	if(setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) == -1)
		fatal("due setting parameter SO_REUSEADDR");

	host_addr.sin_family = AF_INET; //Local order of bytes
	host_addr.sin_port = htons(PORT); //Short integer, net order of bytes
	host_addr.sin_addr.s_addr = 0;

	memset(&(host_addr.sin_zero), '\0', 8); //Nulify left structure

	if(bind(sockfd, (struct sockaddr *)&host_addr, sizeof(struct sockaddr)) == -1)
		fatal("connection with socket");

	if(listen(sockfd, 5) == -1)
		fatal("listening to from socket's side");
	
//	char msg[1024];
	while(1) //Cycle of function accept
	{
		sin_size = sizeof(struct sockaddr_in);
		new_sockfd = accept(sockfd, (struct sockaddr *)&client_addr, &sin_size);
		if(new_sockfd == -1)
			fatal("acception connection");
		printf("server: received connection %s port %d\n", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));
		send(new_sockfd, "Hello, world!\n", 13, 0);
		recv_length = recv(new_sockfd, &buffer, 1024, 0);
		while(recv_length > 0)
		{
			printf("RECV: %d bytes\n", recv_length);
			dump(buffer, recv_length);
			recv_length = recv(new_sockfd, &buffer, 1024, 0);
			if(strstr(buffer, "close") != 0)
				break;
//			scanf("%s", msg);
//			send(new_sockfd, msg, 1024, 0);
		}
		close(new_sockfd);
	}
	return 0;
}
